...
namespace PeterO.Numbers
{
    ...
    internal sealed class DigitShiftAccumulator : IShiftAccumulator
    {
        ...
        public EInteger ShiftedInt
        {
            get
            {
                return this.isSmall ? ((EInteger)this.shiftedSmall) : this.shiftedBigInt;
            }
        }
        ...
        {
            this.knownDigitLength = this.knownDigitLength ?? this.CalcKnownDigitLength();
        ...
        public void ShiftToDigits(FastInteger bits, FastInteger preShift, bool truncate)
        {
            #if DEBUG
            if (bits.Sign < 0)
            {
            ...
            else
            {
                ...
                {
                    // current length is greater than the desired bit length
        ...
        {
            // 'Truncate' is true if the caller doesn't care about the exact identity of the last digit and the discarded digits.
            if (fastint == null)
            {
                throw new ArgumentNullException(nameof(fastint));
            }
        ...
        {
            // Assumes the string is length 9 or less and contains only the digits '0' through '9'
            if (length > 9)
            {
        ...
        private void ShiftRightBig(int digits, bool truncate)
        {
            ...
            if (truncate)
            {
                ...
                if (digits > 50)
                {
                    // To avoid having to calculate a very big power of 10, check the digit count to see if doing so can be avoided
                    EInteger bigBitLength = this.shiftedBigInt.GetUnsignedBitLengthAsEInteger();

                    // NOTE: Overflowing bigBitLength will be MaxValue, which is OK for the use of this variable
                    int bitLength = bigBitLength.CanFitInInt32() ? bigBitLength.ToInt32Checked() : Int32.MaxValue;
                    var bigPower = false;
                    // 10^48 has 160 bits; 10^98 has 326; bit length is cheaper to calculate than base-10 digit length
                ...
                this.bitsAfterLeftmost |= this.bitLeftmost;
                this.discardedBitCount = this.discardedBitCount == null ? new FastInteger(digits) : this.discardedBitCount.AddInt(digits);
                if (bigquo.IsZero)
                {
        ...
        private void ShiftRightLong(long shiftedLong, int digits)
        {
            ...
            this.knownDigitLength = new FastInteger(LongDigitLength(shiftedLong));
            if (this.discardedBitCount != null)
            {
                this.discardedBitCount.AddInt(digits);
            }
            ...
            while (digits > 0)
            {
                ...
                {
                    long newShift = (shiftedLong < 43698) ? ((shiftedLong * 26215) >> 18) : (shiftedLong / 10);
        ...
        /// <summary>
        /// Shifts a number until it reaches the given number of digits, gathering information on whether the last digit discarded is set and whether the discarded digits to the right of that digit are set. Assumes that the big integer being shifted is positive. 'Truncate' is true if the caller doesn't care about the exact identity of the last digit and the discarded digits.
        /// </summary>
        private void ShiftToDigitsBig(int digits, bool truncate)
        {
            ...
            if (digitDiff.CanFitInInt32())
            {
                ...
                {
                    EInteger radixPower = NumberUtility.FindPowerOfTen(digitDiff.AsInt32() - 1);
        ...
        private void ShiftToDigitsSmall(int digits)
        {
            ...
            if (kb > digits)
            {
                ...
                this.UpdateKnownLengthInt(digitShift);
                this.discardedBitCount = this.discardedBitCount != null ?
                this.discardedBitCount.AddInt(digitShift) : new FastInteger(digitShift);
        ...
        private void TruncateRightLong(long shiftedLong, int digits)
        {
            ...
            if (digits >= 1 && digits <= TenPowersLong.Length - 1)
            {
                if (shiftedLong >= TenPowersLong[digits])
                {
                    ...
                    this.bitsAfterLeftmost |= this.bitLeftmost;
                    this.bitLeftmost = (shiftedLong & 1) == 1 ? 1 : (shiftedLong - (quo * bigPower) == 0 ? 0 : 1);
                    shiftedLong = quo;
                    this.isSmall = shiftedLong <= Int32.MaxValue;
                    if (this.isSmall)
                    {
